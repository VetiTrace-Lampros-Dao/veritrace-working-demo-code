...
function getAddress(config) {
  assert4(config);
  const hash5 = keccak2562(concat2(fromString2(accountDomain), padLeft2(config.salt ?? zeroSalt, 32), fromNumber2(config.threshold, { size: 1 }), fromNumber2(config.owners.length, { size: 1 }), ...config.owners.flatMap((owner) => [
    owner.owner,
...
function getSignPayload3(value) {
  ...
  return keccak2562(concat2(fromString2(signatureDomain), from2(payload), account));
}
...
function sortMultisigApprovals(value) {
  ...
  const digest = getSignPayload3("genesisConfig" in value && value.genesisConfig ? { payload, genesisConfig: value.genesisConfig } : { payload, account: value.account });
...
async function resolveNode() {
  ...
  const { wasmBase64 } = await import("./mine.wasm-BCSOSRKA.js");
...
__export(Addresses_exports, {
  ...
  storageCredits: () => storageCredits2,
...
var storageCredits2 = "0x1060000000000000000000000000000000000000";
...
function resolveCallParameters(parameters2) {
  if (parameters2.length === 2)
    return parameters2;
  return [void 0, parameters2[0]];
}
...
*/
//# sourceMappingURL=chunk-H7KB65XD.js.map
